...
let sampleCount = 0;
let distance = 2.5;
let eye = vec3.fromValues(0, 0, distance);
let angleX = 0;
let angleY = 0;
let light = vec3.fromValues(0.4, 0.5, -0.6);
let sphere1 = {
...
let screen00 = vec4.fromValues(-1, -1, 0, 1);
let screen01 = vec4.fromValues(-1, +1, 0, 1);
let screen10 = vec4.fromValues(+1, -1, 0, 1);
let screen11 = vec4.fromValues(+1, +1, 0, 1);
const registerControls = (canvas, eye) => {
...
function main() {
    ...
    let projectionMatrix = mat4.perspective(mat4.create(), Math.PI * 55 / 180, aspectRatio, 0.1, 100);
    let vertices = [
        -1, -1,
        -1, +1,
        +1, -1,
    ...
    function render(now) {
        ...
        gl.clearDepth(1.0);
        gl.clear((gl === null || gl === void 0 ? void 0 : gl.COLOR_BUFFER_BIT) | (gl === null || gl === void 0 ? void 0 : gl.DEPTH_BUFFER_BIT));
        gl.useProgram(programInfo.tracerProgram.program);
        ...
        gl.uniform3fv(programInfo.tracerProgram.uniformLocations.eye, eye);
        let viewMatrix = mat4.lookAt(mat4.create(), eye, vec3.fromValues(0, 0, 0), up);
        ...
        // );
        let ray00_i1 = vec4.transformMat4(vec4.create(), screen00, inverse);
        let ray01_i1 = vec4.transformMat4(vec4.create(), screen01, inverse);
        let ray10_i1 = vec4.transformMat4(vec4.create(), screen10, inverse);
        let ray11_i1 = vec4.transformMat4(vec4.create(), screen11, inverse);
        ...
        let ray00 = vec3.subtract(vec3.create(), vec3.fromValues(ray00_i2[0], ray00_i2[1], ray00_i2[2]), eye);
        let ray01 = vec3.subtract(vec3.create(), vec3.fromValues(ray01_i2[0], ray01_i2[1], ray01_i2[2]), eye);
        let ray10 = vec3.subtract(vec3.create(), vec3.fromValues(ray10_i2[0], ray10_i2[1], ray10_i2[2]), eye);
        let ray11 = vec3.subtract(vec3.create(), vec3.fromValues(ray11_i2[0], ray11_i2[1], ray11_i2[2]), eye);
        ...
        let textureWeight = sampleCount / (sampleCount + 1);
